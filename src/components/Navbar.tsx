          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`nav-link text-sm font-medium py-2 ${location.pathname === link.path ? 'text-primary' : ''}`}
              >
                {link.name}
              </Link>
            ))}
          ...
          <div className="px-4 py-6 space-y-4">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`block text-muted-foreground hover:text-foreground transition-colors py-2 ${location.pathname === link.path ? 'text-foreground' : ''}`}
            ...
            <Link
              ...
              onClick={() => setIsMobileMenuOpen(false)}
            >
              <span className="glow-button inline-block text-sm mt-4">
                Get Started
              </span>
            </Link>
          </div>
